...
# Then edit it: nano /var/www/DetailerFlow/.env

# 3. Verify .env has these CRITICAL fields set:
grep -E "DEBUG|DJANGO_ENV|SECRET_KEY|DATABASE_URL|USE_POSTGRES" /var/www/DetailerFlow/.env
