...
curl -X POST https://www.detely.com/api/token/ \
...
curl -i -X PATCH \
  ...
  https://www.detely.com/api/bookings/$BOOKING_UUID/update_status/

# Step 4: Check if media files are loading (should be 200, not 404)
curl -i https://www.detely.com/media/vehicle_photos/test.jpg

# Step 5: Check signatures endpoint
curl -i https://www.detely.com/media/signatures/test.png
