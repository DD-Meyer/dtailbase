...
curl -X POST https://www.dtailbase.com/api/token/ \
...
curl -i -X PATCH \
  ...
  https://www.dtailbase.com/api/bookings/$BOOKING_UUID/update_status/

# Step 4: Check if media files are loading (should be 200, not 404)
curl -i https://www.dtailbase.com/media/vehicle_photos/test.jpg

# Step 5: Check signatures endpoint
curl -i https://www.dtailbase.com/media/signatures/test.png
